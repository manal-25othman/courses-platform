...
| Date | Ref | Decision |
|---|---|---|
| 2026-09-02 | §51 | **Source rights are resolved.** The client has confirmed she holds the rights and authorisation to use and publish the curriculum content entered into the platform. The hold recorded on 2026-08-31 is lifted: rights are no longer a blocker to publication or to deployment. Three things do not change with it — the approved curriculum content itself is untouched by this decision, every record keeps its `source_ref` and its provenance for maintenance and auditing, and publication remains a separate, explicit instruction rather than something this confirmation performs |
| 2026-09-02 | §17, §18 | **A unit's assessment draws from that unit's own approved questions.** Where a teacher has curated a pool by moving questions into the assessment, that pool *is* the assessment; where she has not, the assessment draws from the unit's approved activity questions. Either way an attempt asks `assessment.question_count` (10) of them, drawn by the engine's seeded shuffle, so no question repeats inside one attempt, none comes from another unit, and none still awaiting a teacher's answer is ever asked. This replaces the earlier requirement to move questions by hand before a unit had an assessment |
| 2026-09-02 | §17 | **Ten questions per assessment.** With the confirmed 80% pass mark each question is worth exactly 10%, so passing is eight right out of ten — a figure a student and a teacher can both read without arithmetic. Held in the settings store, not in code |
| 2026-09-02 | §37.7 | **The teacher's curriculum decisions are recorded in `tooling/content-import/teacher-decisions.json`.** Her answers, her approved source corrections and her confirmation of the grammar pairings live in that file; the script that applies them checks each record against the state the file assumes and refuses rather than forcing a change that no longer matches |
| 2026-08-31 | §21, §22 | **Sequential unlock is built and enforced on the server.** `AssessmentState.blockedBecause` carries `vocabulary_incomplete` and `grammar_incomplete`; `startActivity` and `markSectionViewed` refuse for the same reasons, so a direct address or an API call is blocked exactly as the locked tab is. A component with nothing published in it counts as satisfied, so an unfinished unit cannot become a dead end. Whether the sequence applies is `learning.sequential_unlock`, a setting rather than a rule in code |
| 2026-08-31 | §6 | **Optional grammar video: address only, allow-listed hosts.** A grammar section stores the address a teacher types and nothing else; the player is built from its parts in the API, so teacher-supplied markup is never rendered. Permitted hosts live in `grammar.video_allowed_hosts` (YouTube and Google Drive, both free). No video-hosting dependency and no recurring cost |
| 2026-08-31 | §13.1, §56 | **Bonus games record nothing, by construction.** `GamesService` has no write path: it reads vocabulary and returns a round. There is no attempt row it could create, so a game cannot spend one of the two assessment tries or move any progress. Which games exist is the `bonus_game_types` registry |
| 2026-08-31 | §37.7 | **Unit boundaries in the source are read from the inline vocabulary tables, never from the headings.** Every heading in the supplied file is a floating text box whose position in the XML is not its position on the page, which is what filed seven Professions questions under Grammar Review. A unit with no inline landmark can be attributed nothing and is reported rather than guessed at |
